...
class BridgeRuntimeSmokeTest {
    ...
    private fun startBridgeProcess(isolatedPluginDir: Path, workspaceRoot: String): BridgeProcessHandles {
        val bridgeServerPath = isolatedPluginDir.resolve("jetbrains-ide-bridge").resolve("server.js")
    ...
    private fun findPackagedPluginDir(projectDir: Path): Path {
        ...
        Files.list(sandboxDir).use { products ->
            val match = products
                .filter { Files.isDirectory(it) }
                .map { it.resolve("plugins").resolve("lanes-jetbrains-ide") }
                .filter { Files.isDirectory(it) && Files.isRegularFile(it.resolve("jetbrains-ide-bridge").resolve("server.js")) }
                .findFirst()
        ...
        fail("Could not locate packaged lanes-jetbrains-ide plugin under $sandboxDir")
    }
