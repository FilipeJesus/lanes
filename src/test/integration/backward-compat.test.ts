...
suite("Backward Compatibility", () => {
	...
	suite("Legacy Session Data", () => {
		test("ClaudeCodeAgent parseSessionData handles legacy data without agentName field", () => {
			...
			assert.ok(
				result !== null,
				"parseSessionData should return non-null for valid legacy data",
			);
			assert.strictEqual(
				result!.sessionId,
				"a1b2c3d4-e5f6-7890-abcd-ef1234567890",
				"sessionId should match input",
			);
			assert.strictEqual(
				result!.agentName,
				"claude",
				"agentName should default to claude for ClaudeCodeAgent",
			);
		});
		...
		test("CodexAgent parseSessionData handles data without agentName", () => {
			...
			assert.ok(
				result !== null,
				"parseSessionData should return non-null for valid data",
			);
			assert.strictEqual(
				result!.agentName,
				"codex",
				"agentName should default to codex for CodexAgent",
			);
		});
		...
		test("CodexAgent parseSessionData rejects non-UUID sessionId", () => {
			...
			assert.strictEqual(
				result,
				null,
				"parseSessionData should return null for non-UUID sessionId",
			);
		});
		...
		test("ClaudeCodeAgent still produces correct hook configs", () => {
			...
			assert.ok(
				hooks.length > 0,
				"generateHooksConfig should return non-empty array",
			);
			...
			assert.ok(
				sessionStartHook!.commands.length > 0,
				"SessionStart hook should have commands",
			);
		});
	});
	...
	suite("Agent Coexistence", () => {
		test("Claude and Codex agents have distinct terminal names", () => {
			...
			assert.strictEqual(
				claudeName,
				"Claude: test-session",
				'Claude terminal name should include "Claude:"',
			);
			assert.strictEqual(
				codexName,
				"Codex: test-session",
				'Codex terminal name should include "Codex:"',
			);
			assert.notStrictEqual(
				claudeName,
				codexName,
				"Terminal names should be distinct",
			);
		});
		...
		test("Claude and Codex agents have distinct terminal icons", () => {
			...
			assert.notStrictEqual(
				claudeIcon.color,
				codexIcon.color,
				"Terminal icon colors should be distinct",
			);
			...
			assert.strictEqual(
				claudeIcon.color,
				"terminal.ansiGreen",
				"Claude icon should be green",
			);
			assert.strictEqual(
				codexIcon.color,
				"terminal.ansiBlue",
				"Codex icon should be blue",
			);
		});
		...
		test("supportsHooks returns true for Claude and false for Codex", () => {
			...
			assert.strictEqual(
				claudeSupportsHooks,
				true,
				"Claude should support hooks (non-empty hook events)",
			);
			assert.strictEqual(
				codexSupportsHooks,
				false,
				"Codex should not support hooks (empty hook events)",
			);
		});
	});
});
