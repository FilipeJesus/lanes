...
suite("UnifiedSettingsService", () => {
    ...
    setup(() => {
        tempDir = fs.mkdtempSync(
            path.join(os.tmpdir(), "lanes-unified-settings-test-"),
        );
    ...
    suite(
        "load() parses settings.yaml and get() returns correct values",
        () => {
    ...
    suite(
        "set() writes values to settings.yaml in correct nested YAML format",
        () => {
            test("creates settings.yaml with nested YAML structure", async () => {
                ...
                assert.ok(
                    ...
                    "settings.yaml should be created",
                );
                ...
                assert.ok(
                    ...
                    "YAML should parse to an object",
                );

                const lanes = parsed["lanes"] as Record<string, unknown>;
                assert.ok(
                    lanes !== null && typeof lanes === "object",
                    "Should have nested lanes key",
                );
            ...
            test("set() with nested key creates correct nested YAML structure", async () => {
                ...
                assert.ok(
                    polling !== null && typeof polling === "object",
                    "Should have nested polling key",
                );
    ...
    suite("getAll() returns all settings merged with defaults", () => {
        test("includes all UNIFIED_DEFAULTS keys when no settings.yaml exists", async () => {
            ...
            for (const key of Object.keys(UNIFIED_DEFAULTS)) {
                assert.ok(
                    key in all,
                    `Key '${key}' should be in getAll() result`,
                );
        ...
        test("overrides defaults with values from settings.yaml", async () => {
            ...
            assert.strictEqual(
                all["lanes.defaultAgent"],
                UNIFIED_DEFAULTS["lanes.defaultAgent"],
            );
            assert.strictEqual(
                all["lanes.polling.quietThresholdMs"],
                UNIFIED_DEFAULTS["lanes.polling.quietThresholdMs"],
            );
        });
    });
    ...
    suite(
        "migrateIfNeeded() migrates from .lanes/config.json to settings.yaml",
        () => {
            test("creates settings.yaml from .lanes/config.json values", async () => {
                ...
                assert.ok(
                    fs.existsSync(settingsPath),
                    "settings.yaml should be created after migration",
                );
            ...
            test("does NOT overwrite existing settings.yaml when it already exists", async () => {
                ...
                assert.strictEqual(
                    content,
                    existingContent,
                    "settings.yaml should not be overwritten",
                );
            });
            ...
            test("original config.json still exists after migration", async () => {
                ...
                assert.ok(
                    ...
                    "config.json should not be deleted after migration",
                );
            });
        },
    );
    ...
    suite(
        "onDidChange() callback is invoked when settings.yaml changes on disk",
        () => {
            test("callback is invoked when settings.yaml is modified", async () => {
                ...
                try {
                    // Allow the native watcher to begin listening before modifying the file.
                    ...
                    await new Promise<void>((resolve, reject) => {
                        ...
                        const poll = setInterval(() => {
                            if (callbackInvoked) {
                                ...
                                reject(
                                    new Error(
                                        "onDidChange callback was not invoked within 2 seconds",
                                    ),
                                );
                            }
                        }, 50);
                    ...
                    assert.ok(
                        callbackInvoked,
                        "Callback should have been invoked after file change",
                    );
                } finally {
                    disposable.dispose();
                }
    ...
    suite("set() creates the .lanes directory if it does not exist", () => {
        test("creates .lanes/settings.yaml including parent directories", async () => {
            ...
            assert.ok(
                fs.existsSync(settingsPath),
                ".lanes/settings.yaml should be created",
            );
