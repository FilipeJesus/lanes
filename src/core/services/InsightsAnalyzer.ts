...
function computeEfficiency(insights: SessionInsights): EfficiencyMetrics {
    ...
    // Cache hit rate
    const totalTokensForCache =
        insights.totalCacheReadTokens + insights.totalInputTokens;
    const cacheHitRate =
    ...
    const tokensPerUserMessage =
        insights.totalUserMessages > 0
            ? totalTokens / insights.totalUserMessages
            : 0;

    // Output/input ratio
    const outputInputRatio =
        insights.totalInputTokens > 0
            ? insights.totalOutputTokens / insights.totalInputTokens
            : 0;

    // Average turn duration
    const averageTurnDurationMs =
        insights.totalAssistantTurns > 0
    ...
    const averageUserMessagesPerConversation =
        insights.sessionCount > 0
...
function detectPatterns(insights: SessionInsights): WorkflowPatterns {
    ...
    // Delegation summary
    const totalDelegations = insights.totalSubAgentDelegations.reduce(
        (sum, d) => sum + d.count,
        0,
    );
    const delegationSummary = insights.totalSubAgentDelegations.map((d) => ({
        ...
        count: d.count,
        percentage:
            totalDelegations > 0 ? (d.count / totalDelegations) * 100 : 0,
    }));
    ...
    const readEditRatio =
        editCount > 0 ? readCount / editCount : readCount > 0 ? Infinity : 0;
...
function analyzeErrors(insights: SessionInsights): ErrorAnalysis {
    ...
    const errorRate =
        totalToolCalls > 0 ? (totalErrors / totalToolCalls) * 100 : 0;
    ...
    for (const error of insights.totalToolErrors) {
        errorCountByTool.set(
            error.tool,
            (errorCountByTool.get(error.tool) || 0) + 1,
        );
    }
    ...
    const mostFailedTool =
        errorsByTool.length > 0 ? errorsByTool[0].tool : null;
...
function generateRecommendations(
    ...
    if (efficiency.averageTurnsPerConversation > 30) {
        ...
        efficiency.averageTurnsPerConversation > 15 &&
        efficiency.averageTurnsPerConversation <= 30
    ) {
        recommendations.push({
            severity: "suggestion",
            category: "patterns",
    ...
    // File hotspots
    if (
        patterns.fileHotspots.length > 0 &&
        patterns.fileHotspots[0].totalOperations > 20
    ) {
        recommendations.push({
            severity: "info",
            category: "patterns",
    ...
    if (patterns.delegationSummary.length > 0) {
        const totalDelegations = patterns.delegationSummary.reduce(
            (sum, d) => sum + d.count,
            0,
        );
        const sorted = [...patterns.delegationSummary].sort(
            (a, b) => b.count - a.count,
        );
