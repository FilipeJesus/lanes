...
`;
    ...
    if (projectSettingsPath) {
        try {
            ...
            // Preserve user hooks while replacing hooks previously generated by Lanes.
            if (hooks !== undefined) {
                const existingHooks = existingSettings.hooks as ClaudeSettings['hooks'] | undefined;
                const lanesMarkers = [sessionFilePath, statusFilePath, hookScriptPath, 'workflow_status']
                ...
                for (const event of new Set([
                    ...
                    const userEntries = (existingHooks?.[event] || []).filter(entry =>
                        !entry.hooks.some(hook =>
                            lanesMarkers.some(marker => hook.command.includes(marker))
                        )
                    );
                    mergedHooks[event] = [...userEntries, ...(hooks[event] || [])];
                }
                (settings as Record<string, unknown>).hooks = mergedHooks;
            }
    ...
    if (codeAgent && permissionMode) {
        Object.assign(settings, codeAgent.getPermissionSettings(permissionMode));
    }
