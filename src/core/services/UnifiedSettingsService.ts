 ...
 *
 * Reads and writes a `.lanes/settings.yaml` file so that VS Code and CLI
 * adapters share the same configuration without duplicating defaults or
 * diverging formats.
 *
...
function nestedToFlat(
    ...
    for (const [key, value] of Object.entries(obj)) {
        ...
        if (
            ...
            Object.assign(
                flat,
                nestedToFlat(value as Record<string, unknown>, fullKey),
            );
        } else {
            flat[fullKey] = value;
        }
...
export class UnifiedSettingsService {
    ...
    async load(repoRoot: string): Promise<void> {
        if (!path.isAbsolute(repoRoot)) {
            throw new Error(
                "UnifiedSettingsService: repoRoot must be an absolute path",
            );
        }
    ...
    async set(section: string, key: string, value: unknown): Promise<void> {
        if (!this.settingsPath) {
            throw new Error("UnifiedSettingsService: call load() before set()");
        }
    ...
    async setMany(
        ...
        if (!this.settingsPath) {
            throw new Error(
                "UnifiedSettingsService: call load() before setMany()",
            );
        }
    ...
    /**
     * Migrate the legacy CLI config into settings.yaml if it does not yet exist.
     * The original file is left intact for older CLI versions.
     *
     * @param repoRoot Absolute path to the repository root.
     */
    async migrateIfNeeded(repoRoot: string): Promise<void> {
        ...
        await atomicWrite(
            settingsPath,
            yamlStringify(migrateCliConfig(cliRaw)),
        );
    }
