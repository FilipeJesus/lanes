/**
 * Configuration Store - JSON-backed configuration for JetBrains IDE plugin
 *
 * Since the bridge doesn't have access to VS Code settings API, it stores
 * configuration in a JSON file at `.lanes/jetbrains-ide-config.json` in the workspace root.
 */
...
export class ConfigStore {
    ...
    constructor(workspaceRoot: string) {
        this.configPath = path.join(workspaceRoot, '.lanes', 'jetbrains-ide-config.json');
    }
